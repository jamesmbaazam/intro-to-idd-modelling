...
- Recall that $\dfrac{dI}{dt} = I (\beta S - \gamma)$

::: {.callout-caution collapse="true" icon=false}
# Question

- What will make $\dfrac{dI}{dt} > 0$? and what will that tell us?
:::

---

- If $\dfrac{dI}{dt} > 0$, the number of infected individuals will increase.

- If $\dfrac{dI}{dt} < 0$, the number of infected individuals will decrease.

---

- $\dfrac{dI}{dt} > 0$ when $S > \dfrac{\gamma}{\beta}$

- This means that initially, $S(0)$ must be greater than $\dfrac{\gamma}{\beta}$ for the epidemic to grow.

- This also means that we require the rate of recovery to be less than the rate of transmission for the epidemic to grow.
...
- Can you define the basic reproduction number, $R_0$?
:::
...
- Mathematically, the basic reproduction number, $R_0$, is the rate at which secondary infections are generated from a single primary infection in a completely susceptible population multiplied by the duration of the infectious period.
