...
-   Compartmental models divide populations into compartments (or groups) based on the individual's infection status [@Blackwood2018a].
-  Compartmental models are mechanistic, meaning they describe processes such the interaction between hosts, biological processes of pathogen, host immune response, and so forth.
-   Individuals move between compartments based on defined transition quantities.
-   Individuals can only be in one compartment at a time.

------------------------------------------------------------------------

::: columns
::: {.column width="40%"}
-   Compartments are based on the [disease status]{style="color:red;"} of individuals.
...
::: columns
::: {.column width="60%"}
- SIR models are used to model diseases that confer immunity after recovery. For example, measles.
- This SIR framework was popularised by Kermack and McKendrick in 1927 [@kermack1927contribution]
  - A must-read paper for anyone interested in infectious disease modelling.
:::

::: {.column width="40%"}
![Face of a boy with measles. Source: https://www.immunize.org/clinical/image-library/measles/](images/measles_boy.jpeg){.lightbox}
:::
:::
...
- The SEIR model extends the SIR model to include an exposed compartment, $E$. The exposed compartment represents individuals who have been infected but are not yet infectious.
