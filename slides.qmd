...
-   Compartmental models:
    - divide populations into compartments (or groups) based on the individual's infection status [@Blackwood2018a].
...
::: columns
::: {.column width="50%"}
- Used to model diseases that confer immunity after recovery. For example, measles.
...
- Considering the SIR model, what will cause $\dfrac{dI}{dt} > 0$?
- How do you interpret it?
:::

---

- Note that $\dfrac{dI}{dt} > 0$ when $S > \dfrac{\gamma}{\beta}$

- Hence, at $t = 0$, $S(0)$ must be greater than $\dfrac{\gamma}{\beta}$ for the epidemic to grow.

- This can be interpreted to mean that we require the rate of recovery to be less than the rate of transmission for the epidemic to grow.
...
# An Overview of the Basic Reproduction Number, $R_0$
...
- Conceptually, $R0$ is given by $$ \text{Probability of infection given contact} \times \\ \text{Number of contacts per unit time} \times \\ \text{Duration of infectiousness}$$, and mathematically,

$$R_0 \propto \dfrac{\text{Infection}}{\text{Contact}} \times \dfrac{\text{Contact}}{\text{Time}} \times \dfrac{\text{Time}}{\text{Infection}}$$

- $R0$ is therefore unitless and dimensionless [@yadav2021statistical].
...
- We will learn how to derive $R_0$ from various model structures.
